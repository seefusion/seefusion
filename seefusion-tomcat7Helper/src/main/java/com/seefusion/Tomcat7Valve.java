 ...
 *
 */
public class Tomcat7Valve implements ContainerListener {

	private static final Logger LOG = Logger.getLogger(Tomcat7Valve.class.getName());
	
    FilterDef filterDef;
    FilterMap filterMap;
    
	public Tomcat7Valve(Container con) {
		LOG.info("SeeFusion Tomcat7 Valve Started");
	...
	public void containerEvent(ContainerEvent evt) {
		// LOG.fine(String.format("****Container Event: %s: %s", evt.getType(), evt.getData()));
	...
	private void addFilterToContainer(Container con) {
		if(con instanceof StandardContext) {
			...
			for(Container child : con.findChildren()) {
				LOG.fine(String.format("Check container %s", child.getName()));
